...
// https://github.com/hantabaru1014/BetterInventoryBrowser/blob/main/RecordDirectoryInfo.cs
namespace InventoryHelper
{
    public class RecordDirectoryInfo : IEquatable<RecordDirectoryInfo>
    {
        ...
        public async Task<RecordDirectory> ToRecordDirectory()
        {
            ...
            {
                CoreSearch.Debug("Returning cached record directory data.");
                return cachedRecordDir;
            }
            CoreSearch.Debug("Attempting to resolve record directory...");
            var rootName = Path.Split('\\')[0];
            var rootRecord = new RecordDirectory(RootOwnerId, rootName, Engine.Current);
            CoreSearch.Debug(rootRecord);

            CoreSearch.Debug($"Resolving path: {Path.Substring(rootName.Length + 1)}");

            var result = rootName != Path ? await rootRecord.GetSubdirectoryAtPath(Path.Substring(rootName.Length + 1)) : rootRecord;
            CoreSearch.Debug("Resolved. Caching now.");
            _cache.TryAdd(this, result);
            CoreSearch.Debug("Cache completed. Returning!");
            return result;
        }
